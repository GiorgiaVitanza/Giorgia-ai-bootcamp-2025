...
cur.executemany(
    "INSERT INTO students (id, first_name, last_name, year_of_birth, gender, email,  assignments) VALUES (?,?,?,?,?,?,?)",
    data,
)
conn.commit()
...
cur.execute(
    "SELECT first_name, last_name FROM students WHERE assignments = (?)",
    max_assignments,
)
print(cur.fetchall())
...
with open(
    "Giorgia-ai-bootcamp-2025\python\giorno8\dataStudentsNoAssignments.csv", "w"
) as fd2:
